...
class AgentDashboard extends ConsumerStatefulWidget {
  ...
  const AgentDashboard({super.key, required this.profile});
...
class _AgentDashboardState extends ConsumerState<AgentDashboard> {
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                _AgentWelcomeHeader(profile: widget.profile),
                ...
                // Compliance banner
                _ComplianceBanner(
                    summaryAsync: ref.watch(complianceSummaryProvider)),
                ...
                _SectionHeader(
                  ...
                  action: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: AppTheme.agentGlow,
  ...
  Future<void> _deleteTenancy(String tenancyGroupId) async {
    final ok =
        await ref.read(deleteTenancyProvider.notifier).delete(tenancyGroupId);
    if (!ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
...
class _AgentWelcomeHeader extends StatelessWidget {
  final UserProfile profile;
  const _AgentWelcomeHeader({required this.profile});
...
// ---------------------------------------------------------------------------
// Compliance banner (mirrors landlord dashboard)
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _SectionHeaderTitle(
            icon: icon, title: title, accentColor: accentColor),
        if (action != null) ...[
          const Spacer(),
          action!,
        ],
...
class _SectionHeaderTitle extends StatelessWidget {
  ...
  final Color? accentColor;
  const _SectionHeaderTitle(
      {required this.icon, required this.title, this.accentColor});
...
class _LoadingState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Center(
        child: Padding(
          padding: EdgeInsets.all(24),
          child: CircularProgressIndicator(color: AppTheme.agentGlow),
        ),
      );
}
